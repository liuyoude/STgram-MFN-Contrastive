...
parser.add_argument('--version', default='Contrastive_STgram_MFN(t=0.05)_ArcFace(m=0.5,s=30)', type=str, help='trail version')
...
def cs_test(args, cs = True, norm=True):
    net = SimCLRv2()
    ...
    with torch.cuda.device(args.gpu_index):
        ...
        ft_net = torch.nn.DataParallel(ft_net, device_ids=[0, 1])
        if args.arcface:
        ...
        trainer = wave_Mel_MFN_tester(data_dir=args.data_dir,
            ...
            for key in recore_dict.keys():
                if key not in max_recore.keys():
                    ...
                    if recore_dict[key] > max_recore[key]:
                        ...
                        w_dict[key] = w
        print(w_dict)
        trainer.test(w_dict, cs=cs, norm=norm, save=True)
...
def fine_tune(args, pretrain_path, epoch=20):
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params':arcface.parameters(), 'lr':args.lr} if args.arcface else None,
    ])
    #
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(train_clf_loader), eta_min=0,
                                                           last_epoch=-1)
...
if __name__ == "__main__":
    ...
    args.t = 0.01
    args.pos_margin = 0.4
    ver = f'ID_Contrastive_STgram_MFN(pos_margin={args.pos_margin},t={args.t},lr={args.con_lr})_' \
          f'b={args.con_batch_size}'
    args.version = ver
    print(args.version)
    preprocess()
    main(args)
    ...
    print(args.version)
    pretrain_path = ver
    fine_tune(args, pretrain_path, epoch=args.con_epochs-1)
    test(args)
